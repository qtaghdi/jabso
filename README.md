...
When web authentication is enabled, add the same `JABSO_DATABASE_URL` to the **Jabso Web** Vercel project as well as the collector project. Better Auth opens its database adapter inside the web serverless function; configuring the URL only on `jabso-server` will make sign-in fail before GitHub can redirect.
